...
1. Create your own Single Page Application
1. Use React and `Create React App`
...
What's a killer App? Look it up. 

Discuss.

Single Page Applications are web pages that act like multipage static sites but are built on top of a single HTML file. These pages don't load or reload new html files. Instead they rewrite the content of the exeisting HTML page to show new content. 
...
Here is what you need to know about React for now: 
...
Use the Terminal to navigate a direcdtory where you will build your first React Project. 

`npx create-react-app first-app`

This command creates a new react starter project using the Create React tool. `first-app` would be the name of your app. 
...
When writing JS expressions (JS code) inside a block of JSX you must wrap is in the curly braces `<h1>{str}</h1>`. 
...
Pass props to your component with attributes: 
...
Usually you'll want to put your components into their own files. Do that with the `Heading` component. 
